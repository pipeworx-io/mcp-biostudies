...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'EBI BioStudies');
}
...
async function getStudy(args: Record<string, unknown>): Promise<unknown> {
  ...
  // The accession is the only input on this URL, so a 404 is the caller's
  // identifier missing, not our defect. The old blanket "study not found" also
  // swallowed 5xx/network failures, which hid outages AND booked caller-side
  // misses as `error` — the tier that means Pipeworx has a bug (fleet #409).
  if (res.status === 404) {
    return { error: 'user_error', message: studyMissMessage(accession), accession };
  }
  if (!res.ok) {
  ...
  try {
    ...
    throw new Error(`BioStudies answered HTTP ${res.status} with a body that is not JSON.`);
  }
...
// The production miss pattern is cross-namespace ID confusion: callers hand
// BioStudies a GEO/ArrayExpress accession (E-GEOD-75310) it does not hold.
// Name the expected shape instead of a bare "study not found" (fleet #409).
function studyMissMessage(accession: string): string {
  if (/^(E-|GSE\d|GDS\d)/i.test(accession)) {
    return `BioStudies has no S-* accession matching "${accession}". That looks like a GEO/ArrayExpress accession; BioStudies accessions start with S- (e.g. S-EPMC4300266). Search by keyword with biostudies_search_studies to find one.`;
  }
  return `BioStudies has no study "${accession}". BioStudies accessions start with S- (e.g. S-EPMC4300266). Search by keyword with biostudies_search_studies to find one.`;
}
